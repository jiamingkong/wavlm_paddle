...
from .configuration_wavlm_paddle import WavLMConfig
from .functional import multi_head_attention_forward_paddle

_HIDDEN_STATES_START_POSITION = 2

def paddle_int_div(tensor1, tensor2):
    return paddle.floor_divide(tensor1, tensor2)
...
class WavLMNoLayerNormConvLayer(nn.Layer):
...
class WavLMLayerNormConvLayer(nn.Layer):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class WavLMGroupNormConvLayer(nn.Layer):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class WavLMPositionalConvEmbedding(nn.Layer):
...
class WavLMSamePadLayer(nn.Layer):
...
class WavLMFeatureEncoder(nn.Layer):
...
class WavLMFeatureProjection(nn.Layer):
    def __init__(self, config):
        super().__init__()
...
class WavLMAttention(nn.Layer):
    ...
    def __init__(
        ...
        self.gru_rel_pos_const = self.create_parameter(
            shape=(1, self.num_heads, 1, 1), default_initializer=nn.initializer.Constant(1.0)
        )
    ...
    def forward(
        ...
        gate_a, gate_b = paddle.sigmoid(relative_position_proj).chunk(2, axis=-1)
        ...
        gated_position_bias = gated_position_bias.reshape((-1, tgt_len, tgt_len))
    ...
    def torch_multi_head_self_attention(
        ...
        query = key = value = hidden_states.transpose([1, 0] + list(range(2, len(hidden_states.shape))))
        ...
        attn_output, attn_weights = multi_head_attention_forward_paddle(
            query,
            key,
            value,
            self.embed_dim,
            ...
            paddle.concat([self.q_proj.bias, self.k_proj.bias, self.v_proj.bias]),
            bias_k,
            bias_v,
            add_zero_attn,
            self.dropout,
    ...
    def _relative_positions_bucket(self, relative_positions):
        ...
        relative_positions_if_large = paddle.log(relative_positions.astype("float32") / max_exact)
        relative_positions_if_large = relative_positions_if_large / math.log(self.max_distance / max_exact)
        relative_positions_if_large = relative_positions_if_large * (num_buckets - max_exact)
        # relative_position_if_large = (max_exact + relative_positions_if_large).to(torch.long)
        relative_position_if_large = (max_exact + relative_positions_if_large).astype("int64")
        ...
        relative_buckets += paddle.where(is_small, relative_positions, relative_position_if_large)
        return relative_buckets


# Copied from transformers.models.wav2vec2.modeling_wav2vec2.Wav2Vec2FeedForward with Wav2Vec2->WavLM
class WavLMFeedForward(nn.Layer):
    def __init__(self, config):
        super().__init__()
...
class WavLMEncoderLayer(nn.Layer):
...
class WavLMEncoderLayerStableLayerNorm(nn.Layer):
    def __init__(self, config: WavLMConfig, has_relative_position_bias: bool = True):
        super().__init__()
        self.attention = WavLMAttention(
            embed_dim=config.hidden_size,
...
class WavLMEncoderStableLayerNorm(nn.Layer):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.pos_conv_embed = WavLMPositionalConvEmbedding(config)
...
class WavLMGumbelVectorQuantizer(nn.Layer):
    """
    ...
    def __init__(self, config):
        ...
        # )
        self.codevectors = self.create_parameter(
            shape=(1, self.num_groups * self.num_vars, config.codevector_dim // self.num_groups),
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            # )
            codevector_probs = paddle.zeros_like(hidden_states).scatter_(
                -1, codevector_idx.view(-1, 1), 1.0
            )
...
class WavLMAdapter(nn.Layer):
    def __init__(self, config):
        ...
        self.layers = nn.LayerList(WavLMAdapterLayer(config) for _ in range(config.num_adapter_layers))
...
class WavLMAdapterLayer(nn.Layer):
    def __init__(self, config):
        super().__init__()
        # self.conv = nn.Conv1d(
...
class WavLMPreTrainedModel(nn.Layer):
    ...
    def _get_feat_extract_output_lengths(
        ...
        def _conv_out_length(input_length, kernel_size, stride):
            ...
            return paddle_int_div(input_length - kernel_size, stride) + 1
    ...
    def _get_feature_vector_attention_mask(
        self, feature_vector_length, attention_mask, add_adapter=None
    ):
        # Effectively attention_mask.sum(-1), but not inplace to be able to run
        # on inference mode.
        # non_padded_lengths = attention_mask.cumsum(dim=-1)[:, -1]
        non_padded_lengths = paddle.cumsum(attention_mask, axis=-1)[:, -1]

        output_lengths = self._get_feat_extract_output_lengths(non_padded_lengths, add_adapter=add_adapter)
        # output_lengths = output_lengths.to(torch.long)
        output_lengths = output_lengths.astype("int64")
        ...
        attention_mask = paddle.flip(attention_mask, [-1]).cumsum(-1).flip([-1]).astype("bool")
...
class WavLMModel(WavLMPreTrainedModel):
    def __init__(self, config: WavLMConfig):
        ...
        if config.mask_time_prob > 0.0 or config.mask_feature_prob > 0.0:
            ...
            self.masked_spec_embed = paddle.create_parameter(
                shape=(config.hidden_size,), dtype="float32", default_initializer=nn.initializer.Uniform()
            )
    ...
    def _mask_hidden_states(
        ...
        if mask_time_indices is not None:
            ...
            # mask_time_indices = paddle.Tensor(mask_time_indices, device=hidden_states.device, dtype=torch.bool)
            mask_time_indices = paddle.to_tensor(mask_time_indices, device=hidden_states.device, dtype="bool")
            hidden_states[mask_time_indices] = self.masked_spec_embed.to(hidden_states.dtype)
        ...
        if self.config.mask_feature_prob > 0 and self.training:
            ...
            # mask_feature_indices = paddle.Tensor(mask_feature_indices, device=hidden_states.device, dtype=torch.bool)
            mask_feature_indices = paddle.to_tensor(mask_feature_indices, device=hidden_states.device, dtype="bool")
...
class WavLMForCTC(WavLMPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # )
            attention_mask = (
                attention_mask if attention_mask is not None else paddle.ones_like(input_values, dtype=paddle.long)
            )
            # input_lengths = self._get_feat_extract_output_lengths(attention_mask.sum(-1)).to(torch.long)
            input_lengths = self._get_feat_extract_output_lengths(attention_mask.sum(-1)).astype(paddle.long)
            ...
            log_probs = paddle.nn.functional.log_softmax(logits, axis=-1, dtype=paddle.float32).transpose([1, 0])
...
class WavLMForSequenceClassification(WavLMPreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_weighted_layer_sum:
            ...
            self.layer_weights = paddle.create_parameter(shape=[num_layers], dtype=paddle.float32, default_initializer=paddle.nn.initializer.Constant(1.0 / num_layers))
...
class WavLMForAudioFrameClassification(WavLMPreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_weighted_layer_sum:
            # self.layer_weights = nn.Parameter(torch.ones(num_layers) / num_layers)
            self.layer_weights = self.create_parameter(shape=[num_layers], default_initializer=nn.initializer.Constant(1 / num_layers))
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_fct(logits.reshape([-1, self.num_labels]), paddle.argmax(labels.reshape([-1, self.num_labels]), axis=1))
...
class AMSoftmaxLoss(nn.Layer):
    def __init__(self, input_dim, num_labels, scale=30.0, margin=0.4):
        ...
        self.weight = self.create_parameter(shape=[input_dim, num_labels], default_initializer=nn.initializer.Normal(0, 0.02))
    ...
    def forward(self, hidden_states, labels):
        ...
        cos_theta = paddle.matmul(hidden_states, weight)
        psi = cos_theta - self.margin

        onehot = nn.functional.one_hot(labels, self.num_labels)
        # logits = self.scale * torch.where(onehot.bool(), psi, cos_theta)
        logits = self.scale * paddle.where(onehot.astype("bool"), psi, cos_theta)
...
class WavLMForXVector(WavLMPreTrainedModel):
    def __init__(self, config):
        ...
        self.tdnn = nn.LayerList(tdnn_layers)
    ...
    def forward(
        ...
        if self.config.use_weighted_layer_sum:
            hidden_states = outputs[_HIDDEN_STATES_START_POSITION]
            # hidden_states = torch.stack(hidden_states, dim=1)
            hidden_states = paddle.concat(hidden_states, axis=1)
            norm_weights = nn.functional.softmax(self.layer_weights, dim=-1)
            hidden_states = (hidden_states * norm_weights.view(-1, 1, 1)).sum(dim=1)
        else:
            hidden_states = outputs[0]
        ...
        if attention_mask is None:
            ...
            mean_features = paddle.concat(mean_features, axis=0)
            # std_features = torch.stack(std_features)
            std_features = paddle.concat(std_features, axis=0)
        # statistic_pooling = torch.cat([mean_features, std_features], dim=-1)
        statistic_pooling = paddle.concat([mean_features, std_features], axis=-1)
